...
int smbus_read_byte_data(unsigned bus_number, uint8_t i2c_address, uint8_t register_val, uint8_t *value)
{
    ...
    msg = (i2c_recv_data_msg_t*)malloc(sizeof(struct i2c_recv_data_msg_t) + MIN_READ_BYTES); // allocate enough memory for both the calling information and received data
    if (!msg)
    {
        perror("alloc failed");
...
int smbus_read_block_data(unsigned bus_number, uint8_t i2c_address, uint8_t register_val, uint8_t *block_buffer, uint8_t block_size)
{
    ...
    // Allocate memory for the message
    struct i2c_recv_data_msg_t *msg = NULL;
    msg = (i2c_recv_data_msg_t*)malloc(sizeof(struct i2c_recv_data_msg_t) + block_size); // allocate enough memory for both the calling information and received data
    if (!msg)
    {
        perror("alloc failed");
        return I2C_ERROR_ALLOC_FAILED;
...
int smbus_write_byte_data(unsigned bus_number, uint8_t i2c_address, uint8_t register_val, const uint8_t value)
{
    ...
    msg = (i2c_send_data_msg_t*)malloc(sizeof(struct i2c_send_data_msg_t) + MIN_WRITE_BYTES); // allocate enough memory for both the calling information and received data
    if (!msg)
    {
        perror("alloc failed");
        return I2C_ERROR_ALLOC_FAILED;
...
int smbus_write_block_data(unsigned bus_number, uint8_t i2c_address, uint8_t register_val, const uint8_t *block_buffer, uint8_t block_size)
{
    ...
    // Allocate memory for the message
    struct i2c_send_data_msg_t *msg = NULL;
    msg = (i2c_send_data_msg_t*)malloc(sizeof(struct i2c_send_data_msg_t) + block_size + 1); // allocate enough memory for both the calling information and received data
    if (!msg)
    {
        perror("alloc failed");
        return I2C_ERROR_ALLOC_FAILED;
...
int smbus_write_byte(unsigned bus_number, uint8_t i2c_address, const uint8_t value)
{
    ...
    // Allocate memory for the message
    struct i2c_send_data_msg_t *msg = NULL;
    msg = (i2c_send_data_msg_t*)malloc(sizeof(struct i2c_send_data_msg_t) + MIN_RAW_WRITE_BYTES); // allocate enough memory for both the calling information and received data
    if (!msg)
    {
        perror("alloc failed");
        return I2C_ERROR_ALLOC_FAILED;
...
int smbus_write_block(unsigned bus_number, uint8_t i2c_address, const uint8_t *block_buffer, uint8_t block_size)
{
    ...
    // Allocate memory for the message
    struct i2c_send_data_msg_t *msg = NULL;
    msg = (i2c_send_data_msg_t*)malloc(sizeof(struct i2c_send_data_msg_t) + block_size); // allocate enough memory for both the calling information and received data
    if (!msg)
    {
        perror("alloc failed");
        return I2C_ERROR_ALLOC_FAILED;
